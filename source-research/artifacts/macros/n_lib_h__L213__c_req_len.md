...
- Macro aliases/wrappers: literal `3`, paired with `c_req = "req"`.
...
Provides the compile-time byte length for the paired global constant string declared in `n_lib.h` and defined in `n_const.c`. No exact production `_len` call sites found; paired string is used as the request field key.
...
No exact production `_len` call sites found; paired string is used as the request field key.
...
- Graph node id: `macro:n_lib.h:213:c_req_len`
