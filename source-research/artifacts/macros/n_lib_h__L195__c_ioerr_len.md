...
- Macro aliases/wrappers: literal `4`, paired with `c_ioerr = "{io}"`.
...
Provides the compile-time byte length for the paired global constant string declared in `n_lib.h` and defined in `n_const.c`. No exact production `_len` call sites found; paired string is used for I/O error tags.
...
No exact production `_len` call sites found; paired string is used for I/O error tags.
...
- Graph node id: `macro:n_lib.h:195:c_ioerr_len`
