...
- Macro aliases/wrappers: conditional counterpart at `note.h:78`.
- Signature/API shape: `ERRSTR(x, y)`
- Visibility: available to all translation units including `note.h`; this node is the `NOTE_C_LOW_MEM` definition.
...
- Data/type/global dependencies: depends on compile-time `NOTE_C_LOW_MEM`.
- Artifact coverage: paired normal-mode definition artifact exists at `artifacts/macros/note_h__L78__ERRSTR.md`.

## Behavior

Under `NOTE_C_LOW_MEM`, `ERRSTR(x, y)` expands to `(y)`, selecting compact constant strings such as `c_bad`, `c_err`, `c_mem`, or transport error constants. Without `NOTE_C_LOW_MEM`, the paired definition at `note.h:78` expands to `(x)`, selecting verbose string literals or dynamic string expressions.
...
- Security or parsing constraints: low-memory builds intentionally lose verbose diagnostic detail.
...
- Graph node id: `macro:note.h:75:ERRSTR`
