...
- Public/exported names: `SYNCSTATUS_LEVEL_ALL`
...
Defines the public maximum level value for `NoteDebugSyncStatus` callers requesting all sync status because `NoteDebugSyncStatus` treats negative max level as unlimited. The implementation prints a queued sync note when `maxLevel < 0 || JGetInt(body, "level") <= maxLevel`.
...
- Graph node id: `macro:note.h:414:SYNCSTATUS_LEVEL_ALL`
