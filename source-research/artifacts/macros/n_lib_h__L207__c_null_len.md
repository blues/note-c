...
- Macro aliases/wrappers: literal `4`, paired with `c_null = "null"`.
...
Provides the compile-time byte length for the paired global constant string declared in `n_lib.h` and defined in `n_const.c`. Used by `_parse_value` to match `null` and by `_print_value` to allocate/copy the printed token.
...
Used by `_parse_value` to match `null` and by `_print_value` to allocate/copy the printed token.
...
- Graph node id: `macro:n_lib.h:207:c_null_len`
