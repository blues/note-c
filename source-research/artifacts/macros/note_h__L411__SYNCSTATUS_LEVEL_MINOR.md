...
- Public/exported names: `SYNCSTATUS_LEVEL_MINOR`
...
Defines the public maximum level value for `NoteDebugSyncStatus` callers requesting minor and major sync status. The implementation prints a queued sync note when `maxLevel < 0 || JGetInt(body, "level") <= maxLevel`.
...
- Graph node id: `macro:note.h:411:SYNCSTATUS_LEVEL_MINOR`
