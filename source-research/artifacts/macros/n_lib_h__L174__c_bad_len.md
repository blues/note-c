...
- Macro aliases/wrappers: literal `3`, paired with `c_bad = "bad"`.
...
Provides the compile-time byte length for the paired global constant string declared in `n_lib.h` and defined in `n_const.c`. No exact production `_len` call sites found; paired string is used for bad-request error tokens.
...
No exact production `_len` call sites found; paired string is used for bad-request error tokens.
...
- Graph node id: `macro:n_lib.h:174:c_bad_len`
