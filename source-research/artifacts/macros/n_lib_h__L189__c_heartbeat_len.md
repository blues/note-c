...
- Macro aliases/wrappers: literal `11`, paired with `c_heartbeat = "{heartbeat}"`.
...
Provides the compile-time byte length for the paired global constant string declared in `n_lib.h` and defined in `n_const.c`. No exact production `_len` call sites found; paired string is used to classify heartbeat responses.
...
No exact production `_len` call sites found; paired string is used to classify heartbeat responses.
...
- Graph node id: `macro:n_lib.h:189:c_heartbeat_len`
