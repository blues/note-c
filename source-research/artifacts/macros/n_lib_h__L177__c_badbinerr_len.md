...
- Macro aliases/wrappers: literal `9`, paired with `c_badbinerr = "{bad-bin}"`.
...
Provides the compile-time byte length for the paired global constant string declared in `n_lib.h` and defined in `n_const.c`. No exact production `_len` call sites found; paired string is used to classify bad binary responses.
...
No exact production `_len` call sites found; paired string is used to classify bad binary responses.
...
- Graph node id: `macro:n_lib.h:177:c_badbinerr_len`
