...
- Macro aliases/wrappers: literal `15`, paired with `c_unsupported = "{not-supported}"`.
...
Provides the compile-time byte length for the paired global constant string declared in `n_lib.h` and defined in `n_const.c`. No exact production `_len` call sites found; paired string is used to distinguish unsupported errors from retryable I/O errors.
...
No exact production `_len` call sites found; paired string is used to distinguish unsupported errors from retryable I/O errors.
...
- Graph node id: `macro:n_lib.h:222:c_unsupported_len`
