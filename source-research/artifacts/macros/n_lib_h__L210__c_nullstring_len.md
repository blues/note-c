...
- Macro aliases/wrappers: literal `0`, paired with `c_nullstring = ""`.
...
Provides the compile-time byte length for the paired global constant string declared in `n_lib.h` and defined in `n_const.c`. No exact production `_len` call sites found; paired empty string is returned by `JGetString` for missing/null/non-string values.
...
No exact production `_len` call sites found; paired empty string is returned by `JGetString` for missing/null/non-string values.
...
- Graph node id: `macro:n_lib.h:210:c_nullstring_len`
