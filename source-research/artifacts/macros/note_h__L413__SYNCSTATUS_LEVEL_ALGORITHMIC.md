...
- Public/exported names: `SYNCSTATUS_LEVEL_ALGORITHMIC`
...
Defines the public maximum level value for `NoteDebugSyncStatus` callers requesting algorithmic status up to level 3. The implementation prints a queued sync note when `maxLevel < 0 || JGetInt(body, "level") <= maxLevel`.
...
- Graph node id: `macro:note.h:413:SYNCSTATUS_LEVEL_ALGORITHMIC`
