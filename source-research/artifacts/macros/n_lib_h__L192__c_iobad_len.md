...
- Macro aliases/wrappers: literal `8`, paired with `c_iobad = "bad {io}"`.
...
Provides the compile-time byte length for the paired global constant string declared in `n_lib.h` and defined in `n_const.c`. No exact production `_len` call sites found; paired string is used for I/O badness error tags.
...
No exact production `_len` call sites found; paired string is used for I/O badness error tags.
...
- Graph node id: `macro:n_lib.h:192:c_iobad_len`
