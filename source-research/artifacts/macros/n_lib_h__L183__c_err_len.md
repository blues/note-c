...
- Macro aliases/wrappers: literal `3`, paired with `c_err = "err"`.
...
Provides the compile-time byte length for the paired global constant string declared in `n_lib.h` and defined in `n_const.c`. No exact production `_len` call sites found; paired string is used as the error field key.
...
No exact production `_len` call sites found; paired string is used as the error field key.
...
- Graph node id: `macro:n_lib.h:183:c_err_len`
