...
- Macro aliases/wrappers: literal `3`, paired with `c_cmd = "cmd"`.
...
Provides the compile-time byte length for the paired global constant string declared in `n_lib.h` and defined in `n_const.c`. No exact production `_len` call sites found; paired string is used as the command field key.
...
No exact production `_len` call sites found; paired string is used as the command field key.
...
- Graph node id: `macro:n_lib.h:180:c_cmd_len`
