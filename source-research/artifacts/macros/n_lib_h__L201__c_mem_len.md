...
- Macro aliases/wrappers: literal `3`, paired with `c_mem = "mem"`.
...
Provides the compile-time byte length for the paired global constant string declared in `n_lib.h` and defined in `n_const.c`. No exact production `_len` call sites found; paired string is used for memory error tags.
...
No exact production `_len` call sites found; paired string is used for memory error tags.
...
- Graph node id: `macro:n_lib.h:201:c_mem_len`
