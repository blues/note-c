...
- Kind: macro
- Definition: `note.h:955`
- Declaration/interface: public header macro in `note.h`.
- Public/exported names: `NOTE_C_LOG_LEVEL_INFO`
- Macro aliases/wrappers: literal `2`.
...
Defines an ordered debug/log severity level. `NoteDebugWithLevel` and `NoteDebugWithLevelLn` return early when the message level is greater than runtime `noteLogLevel`. Used by `NOTE_C_LOG_INFO` and `NOTE_C_LOG_LEVEL_DEFAULT`.
...
Focused debug tests cover default INFO ceiling suppressing DEBUG.

## Graph Links

- Graph node id: `macro:note.h:955:NOTE_C_LOG_LEVEL_INFO`
