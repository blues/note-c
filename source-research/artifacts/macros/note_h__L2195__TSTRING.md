...
- Kind: macro
- Definition: `note.h:2195`
- Declaration/interface: public header macro in `note.h`.
- Public/exported names: `TSTRING`
- Macro aliases/wrappers: function-like macro `_NOTE_C_STRINGIZE(N)`
...
fixed-length UTF-8 string template code; stringizes the supplied length token. `NoteTemplate` attaches and sends the caller-owned body unchanged.
...
- Graph node id: `macro:note.h:2195:TSTRING`
