...
- Macro aliases/wrappers: literal `6`, paired with `c_status = "status"`.
...
Provides the compile-time byte length for the paired global constant string declared in `n_lib.h` and defined in `n_const.c`. No exact production `_len` call sites found; paired string is used as the status field key.
...
No exact production `_len` call sites found; paired string is used as the status field key.
...
- Graph node id: `macro:n_lib.h:216:c_status_len`
