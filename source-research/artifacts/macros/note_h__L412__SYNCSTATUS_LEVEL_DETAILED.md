...
- Public/exported names: `SYNCSTATUS_LEVEL_DETAILED`
...
Defines the public maximum level value for `NoteDebugSyncStatus` callers requesting detailed status up to level 2. The implementation prints a queued sync note when `maxLevel < 0 || JGetInt(body, "level") <= maxLevel`.
...
- Graph node id: `macro:note.h:412:SYNCSTATUS_LEVEL_DETAILED`
