...
Finalized. Source-inspected public raw-type predicate.

## Identity

- Kind: function
- Definition: `n_cjson.c:2737`
- Declaration/interface: `n_cjson.h:232`
- Public/exported names: `JIsRaw`
- Macro aliases/wrappers: none found
- Signature/API shape: `Jbool JIsRaw(const J * const item)`
...
Returns `false` when `item` is `NULL`. Otherwise masks `item->type` with `0xFF` and returns true when the base type is `JRaw`.
...
Exact source search found only the public declaration in `n_cjson.h:232` and the definition in `n_cjson.c:2737`. No production repository caller was found.
...
None. Focused test search found no direct `JIsRaw` use.

## Graph Links

- Graph node id: `function:n_cjson.c:2737:JIsRaw`
