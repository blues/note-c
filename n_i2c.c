...
const char *i2cNoteTransaction(char *json, char **jsonResponse)
{
    ...
    while (jsonLen > 0) {
        int chunklen =
            (uint8_t)(jsonLen > (int)_I2CMax() ? (int)_I2CMax() : jsonLen);
        _DelayIO();
    ...
    // Dynamically grow the buffer as we read.  Note that we always put the +1
    // in the alloc so we can be assured that it can be null-terminated, which
    // must be the case because our json parser requires a null-terminated
    // string.
    ...
    while (true) {
        ...
        _DelayIO();
        const char *err =
            _I2CReceive(_I2CAddress(), (uint8_t *)&jsonbuf[jsonbufLen],
                        chunklen, &available);
        if (err != NULL) {
            _Free(jsonbuf);
#ifdef ERRDBG
        ...
        // For the next iteration, read the min of what's available and what
        // we're permitted to read
        chunklen = (int)(available > _I2CMax() ? _I2CMax() : available);

        // If there's something available on the notecard for us to receive, do
        // it
        if (chunklen > 0) {
            continue;
        }
...
bool i2cNoteReset()
{
    ...
    // This outer loop does retries on I2C error, and is simply here for
    // robustness.
    bool notecardReady = false;
    int retries;
    for (retries = 0; transmitErr == NULL && !notecardReady && retries < 3;
         retries++) {
        ...
        while (true) {
            ...
            uint32_t available;
            uint8_t buffer[128];
            chunklen = (chunklen > (int)sizeof(buffer)) ? (int)sizeof(buffer)
                                                        : chunklen;
            chunklen = (chunklen > (int)_I2CMax()) ? (int)_I2CMax() : chunklen;
            _DelayIO();
            const char *err =
                _I2CReceive(_I2CAddress(), buffer, chunklen, &available);
            if (err) {
                break;
            }

            // If nothing left, we're ready to transmit a command to receive the
            // data
            if (available == 0) {
                notecardReady = true;
                break;
