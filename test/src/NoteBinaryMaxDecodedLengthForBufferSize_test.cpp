/*!
 * @file NoteBinaryMaxDecodedLengthForBufferSize_test.cpp
 *
...
SCENARIO("NoteBinaryMaxDecodedLengthForBufferSize")
{
    ...
    GIVEN("Parameters are in order") {
        ...
        const uint32_t result = NoteBinaryMaxDecodedLengthForBufferSize(len);
        ...
        WHEN("cobsGuaranteedFit is invoked") {
            THEN("The parameters are passed without modification") {
                CHECK(cobsGuaranteedFit_fake.arg0_history[0] == len);
            }
